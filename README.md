...
Один и тот же набор вопросов на **SQuAD v2**, базовый генератор галлюцинирует в ~50% случаев.
Сравнение стратегий (воспроизводится из `experiments/run_phase4.py`):

| Подход                                | Доля галлюцинаций | Δ к бейзлайну |
|---------------------------------------|:-----------------:|:-------------:|
| Обычный RAG (без критика)             | 45%               | —             |
| Self-consistency (голосование)        | 11%               | −76%          |
| Best-of-n (rejection sampling)        | 0.8%              | −98%          |
| Цикл самокоррекции                    | 0.8%              | −98%          |
| **Best-of-n + цикл (этот проект)**    | **0%**            | **−100%**     |

> Отдельный end-to-end прогон (`run_real`, 640 примеров): **66.7% → 0.5%** галлюцинаций,
> F1 критика **0.99**, ROC-AUC **1.00**.

**Чем отличается от обычного RAG:**
- 🔁 **Самопроверка перед выдачей** — обученный критик отбраковывает неподкреплённые ответы, а не надеется, что LLM «сама не соврёт».
- 🧪 **Честный стресс-тест** — отдельный набор *сложных* галлюцинаций (пересборка из слов контекста), где лексические методы слепы. Показываю и слабые места, а не только победные графики.
- 🪶 **Дёшево по разметке** — self-training выводит F1 на уровень полного обучения (0.82 ≈ 0.80) при ручной разметке всего ~4% данных.
- ⚙️ **End-to-end и воспроизводимо** — FAISS-ретривер + генератор + цикл коррекции, CI с pytest, все метрики в репо.
